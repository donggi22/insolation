...
def prepare_data(sn):
    file_path = f'preprocessed_{sn}_ver2.csv'
    ...
    df = pd.get_dummies(df, columns=['sky', 'pty'])
    ...
    weather_features = ['reh_weather', 'ws_weather', 'rain_weather'] + \
                   [col for col in df.columns if col.startswith('sky_') or col.startswith('pty_')]

    features_all = [
        'hour_sin', 'hour_cos', 'Pac', 'Pac_diff',
...
def run_comparison(sn):
    ...
    results = {}
    feat_a = ['hour_sin', 'hour_cos', 'Pac', 'light_intensity', 'temperature', 'humidity', 'wind_speed', 'uv_index', 
              'Upv1', 'Ipv1', 'Upv2', 'Ipv2', 'Upv3', 'Ipv3', 'Tmod', 'Tamb', 'PF', 'PF_abnormal'] + weather_features
